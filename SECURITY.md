...
| 解释器访问系统目录 | 使用自包含 embeddable Python | `bootstrap.py` 置于 `%LOCALAPPDATA%\arena_python` |
...
- 首次使用 `native-sandbox` 需联网下载 embeddable Python（约 10MB）；离线时该模式不可用，自动回退 `off`。
...
覆盖：宿主文件读取、目录外写入、网络连接、内存/CPU/进程配额、环境变量不继承、
并发任务的跨任务工作目录隔离（互异 SID）。
