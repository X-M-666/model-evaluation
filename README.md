...
- **四个评测入口**：双模型对决 / N 模型比较（任务页 4 步向导：行内输入 2~20 个模型，1 评测集 × N 模型 × M 轮，单臂评审）/ 文本对战（人工 5 档投票）/ 扰动鲁棒性（迭代十一：集成于评测流程，评测完成自动对每个模型跑扰动评测；**扰动与主流程解耦**——主评测完成即出排行榜，扰动后台执行并在排行榜页动态展示进度与结果；扰动作答 3 路并发提速，噪声注入默认单强度）
...
- **LLM 出题**：生成 → 五级自动校验（去重/可解性/rubric 完整性/防泄漏/内容安全）→ 人工审核 → 版本化入库；**维度可多选（每维度各生成指定数量）**；RAG/grounding 忠实性评测
- **饱和度监测**：跨 job 逐题汇总（幂等），评测集题目饱和提示
- **排行榜形态**：分维度排名 + 综合分 + 胜率矩阵 + bootstrap CI「差异不显著」标注 + 多模型雷达 / 箱线 / 散点 / K-召回率 + **KPI 概览**（迭代十一：原 KPI 看板融入排行榜页，耗时/Token 趋势 + 明细；硬件利用率与 Bad Case 已移除）
- **资源管理**：优先级队列 + 并发配额（`MODEL_DUEL_CONCURRENCY`）+ 排队替代 429 + 断点续跑 + 批次整体取消 + 已终态批次删除（联动清理排行榜与 job 历史）
...
- **Key 不落盘不变量**：模型 Key 全部走请求体内联（被测/评审/出题/embedding）仅存内存，持久化与审计递归脱敏
...
| 页面 | 功能 |
|---|---|
| `/`（index.html） | 入口页（迭代十一）：精致 hero + 「进入评测工作台」按钮，全部功能浓缩到任务页 |
| `/tasks.html` | 评测工作台：访问令牌、排队/运行视图、评测任务表格（搜索/状态筛选/综合得分/删除）、新建任务 4 步向导（验证连通性、评测集管理、扰动评测开关）、LLM 出题、SSE 实时刷新 |
| `/battle.html` | 文本对战：双模型同题并排流式作答（URL 填 /v1 自动拼接端点）、盲评模式（身份隐藏+左右随机+评完揭晓）、手动跳题、会话持久化（跳转/刷新不重置）、5 档人工投票、结果统计弹窗 |
| `/leaderboard.html` | 排行榜：历史 job 聚合 + 查看 + 批次扰动鲁棒性区块（衰减曲线/偏见对照）+ KPI 概览（耗时/Token 趋势，迭代十一融入）；已有排行榜点击跳转查看/可删除 |
| `/gen_review.html` | 出题批次审核（批准/驳回/编辑/删除已完成批次） |
| `/report.html` | 评测报告（指标/图表/元评估/环境快照/导出） |
| `/review.html` | 人工双盲评审（含 Hybrid 复核） |

公共层：`frontend/common.js`（apiFetch/toast/三态/顶栏/轮询）+ `frontend/common.css`（统一布局），8 页统一导航与空/加载/错误态（迭代十一：KPI 看板与 Bad Case 页面已移除，KPI 并入排行榜）。
...
- **纯函数单测**：parsers/datasets/storage/tasks/metrics/embed/extract/stats/perturb/leaderboard/generator/scheduler/长期文本基准等
- **集成测试**：全链路状态机（含 AI 评审/hybrid）、batch 聚合与部分失败、批次取消、断点续跑、排队/取消/优先级、SSE ticket（eval + tasks 双路径）、饱和度幂等、文本对战（抽题/流式/score）、批次遗留自愈（重启后扰动沉降 + 排行榜自动生成）
- **安全回归**：Key 不落盘、SSE ticket 401/204/剥离、代理日志不含长期令牌、SSRF、审计白名单、CSP
- **浏览器 E2E（本地 playwright）**：`test_xss_playwright.py`（页面冒烟/顶栏/数据集注入防护/评审流程/任务页向导）+ `test_battle_playwright.py`（文本对战全流程/盲评与揭晓/任务向导含扰动开关），未安装 playwright 自动跳过，CI 不运行
...
│   ├── storage.py         # 历史/评测集/排行榜/扰动/批次文件库
│   ├── scheduler.py       # 优先级队列 + 并发配额（纯逻辑）
│   ├── access.py          # 认证/Host/Origin/限流/SSE ticket
│   ├── audit.py / security.py / ssrf.py / sse_ticket.py
│   └── engine/
...
│       ├── perturb.py     # 扰动/偏见管线（改写/噪声/属性扰动，集成于评测流程）
│       ├── leaderboard.py # 排行榜聚合（bootstrap CI）
│       ├── battle.py      # 文本对战（迭代十一）：抽题 + 双路 SSE 流式（无状态）
│       ├── rag_demo.py / longtext.py / gold.py  # 内置资产（启动幂等生成）
│       ├── mock.py        # 演示数据（测试基础设施 / demo_seed）
│       └── isolation/     # 代码沙箱（off / Windows 原生）
├── frontend/              # 10 页面 + common.js/css + echarts.min.js
